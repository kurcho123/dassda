TryToFish = function()
    QBCore.Functions.TriggerCallback('qb-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return QBCore.Functions.Notify("Не можете да плувате и да ловите риба едновременно.", "error") end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return QBCore.Functions.Notify("Трябва да излезете от автомобила си, за да започнете риболов.", "error") end 
        if count ~= nil then
            if count == 0 then
                QBCore.Functions.Notify("Имате нужда както от въдица, така и от стръв за риболов.", "primary")
            else
                ...
                if waterValidated then
                    ...
                    QBCore.Functions.Notify("Трябва да се насочите към водата, за да ловите риба", "primary")
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        QBCore.Functions.Notify('Нямаш стръв!', 'error')
    ...
    while not IsControlJustPressed(0, 47) do
        ...
        ShowHelpNotification("Прехвърлете линията си чрез натискане ~INPUT_DETONATE~")

        if GetGameTimer() - startedCasting > 5000 then
            QBCore.Functions.Notify("Трябва да хвърлите стръвта.", "primary")
    ...
    QBCore.Functions.Notify("В очакване на риба да клъвне...", "success", "10000")
    ...
    if caughtFish then
        ...
        QBCore.Functions.Notify("Рибата се измъкна.", "error")
    end
