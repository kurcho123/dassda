				...
				if GetVehicleClass(vehicle) ~= 18 then
					...
					if Config.UseKmh then -- If you are using KMH
						...
						if travelSpeed > Config.SpeedLimitKmh then
							QBCore.Functions.Notify("Бяхте хванати от полицейски радар за превишена скорост.", "primary")
...
RegisterNetEvent('radar:client:SendBillEmail', function(amount)
    SetTimeout(math.random(6500, 8000), function()
        local gender = "господин."
        if QBCore.Functions.GetPlayerData().charinfo.gender == 1 then gender = "госпожице." end
        ...
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = "Полицията",
            subject = "Камера за скорост на КАТ",
            message = "Здравейте, " .. gender .. " " .. charinfo.lastname .. "<br/><br/>Информираме Ви за получаване на глоба за превишена скорост на улица <strong>" .. street1name .." / ".. street2name .. "</strong>.<br/><br/>Скоростта, с която сте сниман е <strong>" .. travelSpeed .. " " .. speedType .. "</strong><br/><br/>Автомобилен номер на ПМС-то е: <strong>" .. plate .. "</strong><br/><br/>Глобени сте с: <strong>$" .. amount .. "</strong><br/>",
        })
    end)
end)
