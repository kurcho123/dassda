    ...
    while listen do
      if IsControlJustPressed(0, 38) then
        ...
        if not Config.MiningLocation[mining]["isMined"] and not Config.MiningLocation[mining]["isOccupied"] then
          ...
          exports['qb-drawtext']:DrawText(Config.Text['error_alreadymined'], 'left')
        end
      end
      Wait(0)
    end
  ...
  for k, v in pairs(Config.MiningLocation) do
    ...
    shaftZones:onPlayerInOut(function(isPointInside)
      if isPointInside then
        exports['qb-drawtext']:DrawText(Config.Text['MiningAlert'], 'left')
        Wait(1500)
        exports['qb-drawtext']:HideText()
        Wait(1000)
        exports['qb-drawtext']:DrawText(Config.Text['StartMining'],'left')
        MiningKeyBind(k)
      else
        listen = false
        exports['qb-drawtext']:HideText()
