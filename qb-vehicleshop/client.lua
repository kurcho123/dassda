...
local vehHeaderMenu = {
    {
        header = 'Каталог с МПС',
        txt = 'Купи си МПС и стани газар',
        params = {
            ...
            CreateThread(function()
                while insideZones[name] do
                    ...
                    vehicleMenu = {
                        ...
                        {
                            header = "Купи МПС-то",
                            txt = 'С тази опция може да купите МПС-то пред вас',
                            params = {
                                isServer = true,
                        ...
                        -- },
                        {
                            header = 'Разгледай други МПС-та',
                            txt = 'С тази опция може да разгледате други МПС-та',
                            params = {
            ...
            CreateThread(function()
                while insideZones[name] and PlayerData.job ~= nil and PlayerData.job.name == Config.Shops[name]['Job'] do
                    ...
                    vehicleMenu = {
                        ...
                        {
                            header = 'Пробвай как върви',
                            txt = 'С тази опция може да разгледате МПС-то',
                            params = {
                        ...
                        {
                            header = "Продай МПС",
                            txt = 'Това МПС явно не е за теб продай го',
                            params = {
                                event = 'qb-vehicleshop:client:openIdMenu',
                                args = {
                                    vehicle = Config.Shops[closestShop]["ShowroomVehicles"][ClosestVehicle].chosenVehicle,
                        ...
                        {
                            header = 'Разгледай други МПС-та',
                            txt = 'С тази опция може да разгледате католага',
                            params = {
                                event = 'qb-vehicleshop:client:vehCategories',
                            }
                        },
...
RegisterNetEvent('qb-vehicleshop:client:TestDrive', function()
    if not inTestDrive and ClosestVehicle ~= 0 then
        ...
        QBCore.Functions.SpawnVehicle(Config.Shops[getShopInsideOf()]["ShowroomVehicles"][ClosestVehicle].chosenVehicle, function(veh)
            ...
            testDriveVeh = veh
            QBCore.Functions.Notify('Брат даваме ти '..Config.Shops[closestShop]["TestDriveTimeLimit"]..' минути')
            SetTimeout(Config.Shops[closestShop]["TestDriveTimeLimit"] * 60000, function()
                if testDriveVeh ~= 0 then
                    ...
                    QBCore.Functions.Notify('Тест  драйва свърши брат')
                end
            end)
        ...
        QBCore.Functions.Notify('Брато, покрай малко', 'error')
    end
end)
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    if Config.Shops[data.ClosestShop]["ShowroomVehicles"][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        ...
        if closestDistance < 2 then QBCore.Functions.DeleteVehicle(closestVehicle) end
        Wait(250)
