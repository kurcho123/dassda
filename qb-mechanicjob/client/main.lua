    ...
    if Config.UseTarget then
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['qb-drawtext']:DrawText("[E] " .. label, 'left')
            else
...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        ...
        zone:onPlayerInOut(function (isPointInside)
            if isPointInside then
                exports['qb-drawtext']:DrawText("[E] Open Stash", 'left')
            else
                exports['qb-drawtext']:HideText()
            end
...
local function RegisterGarageZone()
    ...
    vehicleZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            ...
            if inVehicle then
                exports['qb-drawtext']:DrawText('[E] Hide Vehicle', 'left')
            else
                exports['qb-drawtext']:DrawText('[E] Get Vehicle', 'left')
            end
        else
            exports['qb-drawtext']:HideText()
        end
    ...
    plateZone:onPlayerInOut(function (isPointInside)
        if isPointInside and onDuty then
            if plate.AttachedVehicle then
                exports['qb-drawtext']:DrawText('[E] Open Menu', 'left')
            else
                if IsPedInAnyVehicle(PlayerPedId()) then
                    exports['qb-drawtext']:DrawText('[E] Work On Vehicle', 'left')
                end
            end
        else
            exports['qb-drawtext']:HideText()
...
CreateThread(function ()
    ...
    AddTextComponentSubstringPlayerName("Механици")
