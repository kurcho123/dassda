...
RegisterNetEvent("qb-uwu:bentobox", function()
	...
	if randomToy < 45 then
		QBCore.Functions.Notify("Откраднаха ти играчката...", "uwu")
	...
	elseif randomToy > 45 and randomToy < 53 then
		...
		QBCore.Functions.Notify("Ууу, късметлия!", "uwu")
		...
		QBCore.Functions.TriggerCallback('qb-uwu:server:get:ingredientBentobox', function(HasItems)
			if HasItems then
				QBCore.Functions.Progressbar("pickup_sla", "Preparing a Bento Box..", 4000, false, true, {
					...
					QBCore.Functions.Notify("Приготвено 1x Котешка изненада!", "uwu")
				end, function()
					QBCore.Functions.Notify("Cancelled..", "uwu")
				end)
			else
				QBCore.Functions.Notify("Не сте на смяна!", "uwu")
			end
		end)
	else
...
RegisterNetEvent("qb-uwu:misoSoup", function()
	if onDuty then
		QBCore.Functions.TriggerCallback('qb-uwu:server:get:ingredientMiso', function(HasItems)
			if HasItems then
				QBCore.Functions.Progressbar("pickup_sla", "Приготвено 1x Супа!", 4000, false, true, {
					disableMovement = true,
					disableCarMovement = true,
					disableMouse = false,
					disableCombat = true,
					...
					QBCore.Functions.Notify("Приготвено 1x Супа!", "uwu")
				end, function()
					QBCore.Functions.Notify("Cancelled..", "uwu")
				end)
			else
...
CreateThread(function()
	exports['qb-target']:AddTargetModel(0x573201B8, {
		options = {
			{
				event = 'catcaffe-galene',
				type = 'client',
