	...
	while true do Citizen.Wait(0)
		...
		for k,v in pairs(CONFIG['Machines']) do
			if k == currentMachine and not v.available then
				if v.finished then
					DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~g~E~w~ - Събери.')
					if IsControlJustPressed(0,38) then
						TriggerServerEvent('qb-overlord-laundering:collect', k)
						currentMachine = -1
					end
				else
					DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~r~Пране...')
				end
			else
				if not displayed and #(v.vec - globalCoords) < 1.0 then
					if v.available then
						DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~g~E~w~ - Зареди ($'..v.cost..')')
						if IsControlJustPressed(0,38) then
							TriggerServerEvent('qb-overlord-laundering:load', k)
							currentMachine = k
						end
					else
						DrawText3Ds(v.vec.x, v.vec.y, v.vec.z, '~r~Тази пералня е заета')
					end
