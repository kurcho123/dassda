...
local function smashVitrine(k)
    if validWeapon() then
        ...
        elseif math.random(1, 100) <= 55 and IsWearingHandshoes() then
            TriggerServerEvent("evidence:server:CreateFingerDrop", plyCoords)
        ...
        QBCore.Functions.Progressbar("smash_vitrine", "Looting case", Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
            ...
            TriggerServerEvent('police:server:policeAlert', 'ИЗВЪРШВА СЕ ОБИР НА БИЖУТЕРИЯ 10-90')
            smashing = false
        ...
        QBCore.Functions.Notify('Това, което е в ръката ти е прекалено слабо за тук!..', 'error')
    end
end
        ...
        if result then 
            ...
            QBCore.Functions.TriggerCallback('qb-jewellery:server:getCops', function(cops)
                if cops >= Config.RequiredCops then
                    ...
                    QBCore.Functions.Notify("Няма ченгета на смяна..", "error")
                end
            end)
        else
            QBCore.Functions.Notify("Липсва ти нещо..", "error", 2500)
        end
...
CreateThread(function()
    ...
    AddTextComponentSubstringPlayerName("Бижутериен Магазин")
