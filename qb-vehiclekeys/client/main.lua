            ...
            if result then
                ...
                if vehLockStatus == 1 then
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 2) then
                        ...
                        QBCore.Functions.Notify("Превозното средство е заключено!")
                    else
                        QBCore.Functions.Notify("Нещо се обърка със заключващата система!")
                    end
                else
                    Wait(750)
                    ClearPedTasks(ped)
                    ...
                    if (GetVehicleDoorLockStatus(veh) == 1) then
                        ...
                        Wait(200)
                        SetVehicleLights(veh, 0)
                        QBCore.Functions.Notify("Превозното средство е отключено!")
                    else
                        QBCore.Functions.Notify("Нещо се обърка със заключващата система!")
                    end
                end
            else
                QBCore.Functions.Notify('Нямате ключовете на автомобила..', 'error')
            end
        end, plate)
    end
end
...
local function lockpickFinish(success)
    ...
    if success then
        ...
        QBCore.Functions.Notify('Отворихте врата!', 'success')
        ...
        PoliceCall()
        TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
        QBCore.Functions.Notify('Някой се обади на полицията!', 'error')
    end
...
local function Hotwire()
    if not HasVehicleKey then
        ...
        IsHotwiring = true
        lockpickedPlate = nil
        local hotwireTime = math.random(9000, 21000)
        SetVehicleAlarm(vehicle, true)
        SetVehicleAlarmTimeLeft(vehicle, hotwireTime)
        PoliceCall()
        QBCore.Functions.Progressbar("hotwire_vehicle", "Опитваш да запалиш кола", hotwireTime, false, true, {
            ...
            if (math.random() <= Config.HotwireChance) then
                ...
                QBCore.Functions.Notify("Успяхте да запалите МПС-то!")
            else
...
local function RobVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", "Опит за грабеж..", 6000, false, true, {}, {}, {}, {}, function()
        local chance = math.random()
        if chance <= Config.RobberyChance then
            ...
            QBCore.Functions.Notify('Получихте ключовете!', 'success')
            Wait(10000)
            IsRobbing = false
        else
            PoliceCall()
            ...
            TaskReactAndFleePed(target, PlayerPedId())
            TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            QBCore.Functions.Notify('Обадиха се на ченгетата!', 'error')
            Wait(10000)
            IsRobbing = false
        end
    end)
...
RegisterNetEvent('vehiclekeys:client:GiveKeys', function(target)
    ...
    if vehicles then
        ...
        QBCore.Functions.Notify('Трябва да сте в превозно средство, за да дадете ключ', 'error')
    end
end)
        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and lockpicked and not IsHotwiring and not HasVehicleKey then
                ...
                if GetPedInVehicleSeat(veh, -1) == PlayerPedId() then
                    DrawText3D(vehpos.x, vehpos.y, vehpos.z, "~g~[H]~w~ - Запали МПС-то")
