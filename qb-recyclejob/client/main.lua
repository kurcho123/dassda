...
CreateThread(function()
    ...
    AddTextComponentString("Рециклиране")
    ...
    while true do
        ...
        if #(pos - vector3(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z)) < 1.3 then
            DrawText3D(Config['delivery'].outsideLocation.x, Config['delivery'].outsideLocation.y, Config['delivery'].outsideLocation.z + 1, "~w~[~g~E~w~]~w~ - За да влезеш")
		...
		if #(pos - vector3(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z)) < 1.3 then
			DrawText3D(Config['delivery'].insideLocation.x, Config['delivery'].insideLocation.y, Config['delivery'].insideLocation.z + 1, "~w~[~g~E~w~]~w~ - За да излезеш")
			if IsControlJustReleased(0, 38) then
				DoScreenFadeOut(500)
				while not IsScreenFadedOut() do
					Wait(10)
        ...
        if #(pos - vector3(1049.15, -3100.63, -39.95)) < 15 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            ...
            if #(pos - vector3(1049.15, -3100.63, -39.95)) < 1.3 then
                if onDuty then
                    DrawText3D(1049.15, -3100.63, -38.95, "~w~[~g~E~w~]~w~ - Прекрати работа")
                else
                    DrawText3D(1049.15, -3100.63, -38.95, "~g~E~w~ -  Започни работа")
                end
                if IsControlJustReleased(0, 38) then
                    onDuty = not onDuty
                    if onDuty then
                        QBCore.Functions.Notify("Започнахте работа!", "success")
                    else
                        QBCore.Functions.Notify("Приключихте работа!", "error")
                    end
                end
            end
        end
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) < 2.3 then
                        DrawText3D(packagePos.x,packagePos.y,packagePos.z+ 1, "~w~[~g~E~w~]~w~ - Вземи кашон")
                        if IsControlJustReleased(0, 38) then
                            QBCore.Functions.Progressbar("pickup_reycle_package", "Взимате кашон с боклуци ..", math.random(4000, 6000), false, true, {
                    ...
                    if #(pos - vector3(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z)) < 2.0 then
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "~w~[~g~E~w~]~w~ - Рециклирай")
                        if IsControlJustReleased(0, 38) then
                            DropPackage()
                            ScrapAnim()
                            QBCore.Functions.Progressbar("deliver_reycle_package", "Отваряте кашона", 5000, false, true, {
                                disableMovement = true,
                                disableCarMovement = true,
                                disableMouse = false,
                                disableCombat = true,
                        ...
                        DrawText3D(Config['delivery'].dropLocation.x, Config['delivery'].dropLocation.y, Config['delivery'].dropLocation.z, "Рециклиране")
                    end
                end
            else
                GetRandomPackage()
