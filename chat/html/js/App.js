...
$(document). ready(function() { 
  var commands = ['/report и описваш кратко точно и ясно!','/me']
  $.each(commands,function(k,v){
    var text = v[0].toUpperCase() + v.substring(1)
    if (v == 'ooc'){
      text = v.toUpperCase()
    }
    $('.buttons').append(`<button onclick="$('textarea').val('/'+$(this).attr('id')+' ')" id="${v}">${text}</button>`)
  })
})
