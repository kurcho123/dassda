        ...
        for i = 1, #dumpsters do
            ...
            for i = 1, #searched do
                ...
                if i == #searched and dumpsterFound then
                    QBCore.Functions.Notify('Няма нищо в кофата иди на друга', 'error')
...
RegisterNetEvent('qb-trashsearch:client:progressbar', function(itemType)
    ...
    QBCore.Functions.Progressbar("trash_find", "Ровя в кофата", math.random (15000, 30000), false, true, {
        ...
        if Config.RewardTypes[itemType].type == "item" then
            QBCore.Functions.Notify("Намерихте нещо", "success")
            QBCore.Functions.Notify("Вони ти на лайна кеф ли ти е ")
            TriggerServerEvent('qb-trashsearch:server:recieveItem')
        elseif Config.RewardTypes[itemType].type == "money" then
            QBCore.Functions.Notify("Намери кинти ", "success")
            QBCore.Functions.Notify("Вони ти на лайна кеф ли ти е")
            TriggerServerEvent('qb-trashsearch:server:givemoney', math.random (2, 6))
        elseif Config.RewardTypes[itemType].type == "nothing" then
            QBCore.Functions.Notify("Ненамери нищо", "error")
            QBCore.Functions.Notify("Вони ти на лайна кеф ли ти е")
        end
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "amb@prop_human_bum_bin@idle_b", "idle_d", 1.0)
        QBCore.Functions.Notify("Спряно ровене", "error")
    end)
end)
