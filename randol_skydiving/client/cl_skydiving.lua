...
CreateThread(function()
    ...
    AddTextComponentString("Парашутни скокове")
    ...
    CreateThread(function()
        while waitingTakeOff do
            if GetGameTimer() < gameTimer + tonumber(1000 * Time) then
                ...
                drawTxt("Излитаме след: " .. math.ceil(Time - secondsLeft / 1000), 4, 0.5, 0.93, 0.50, 255, 255, 255, 180)
            end
            Wait(0)
        end
    end)
			...
			options = {
				{
					...
					label = "Самолет + скок ($850)",
				},
				{
					type = "client",
					event = "randol_skydive:client:ridewithbuddy",
					icon = "fa-solid fa-user-group",
					label = "Скачай с приятел",
				},
				{
					type = "server",
...
AddEventHandler('randol_skydive:client:ridewithbuddy', function()
	...
	if foundVehicle == nil then QBCore.Functions.Notify("Все още никой не е платил груповия полет!", 'error') return end
