...
RegisterKeyMapping('mdt', 'Отворете мобилната дата база', 'keyboard', 'k')
...
RegisterCommand('mdt', function()
    ...
    if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
        ...
        QBCore.Functions.Notify("Не можете да извършите това!", "error")
    end
end, false)

Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/mdt', 'Отворете мобилната дата база', {})
end)
...
RegisterNetEvent('mdt:client:open', function(bulletin, activeUnits)
    ...
    SendNUIMessage({ type = "data", activeUnits = activeUnits, name = "Добре дошли, " ..PlayerData.job.grade.name..' '..PlayerData.charinfo.lastname:sub(1,1):upper()..PlayerData.charinfo.lastname:sub(2), location = playerStreetsLocation, fullname = PlayerData.charinfo.firstname..' '..PlayerData.charinfo.lastname, bulletin = bulletin })
    TriggerEvent("mdt:client:dashboardWarrants")
end)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('Поставена е GPS локация!', 'success')
end)
...
RegisterNUICallback("saveVehicleInfo", function(data, cb)
    ...
    if JobType == 'police' and impound.impoundChanged == true then
        if impound.impoundActive then
            ...
            if found == 0 then
                QBCore.Functions.Notify('Автомобилът не е намерен!', 'error')
...
RegisterNetEvent('mdt:client:setRadio', function(radio)
    if type(tonumber(radio)) == "number" then
        ...
        QBCore.Functions.Notify("Зададохте радио честотата си на "..radio..".", "success")
    else
...
RegisterNetEvent('mdt:client:sig100', function(radio, type)
    ...
    if (job == "police" or job == "ambulance") and duty == 1 then
        if type == true then
            exports['erp_notifications']:PersistentAlert("START", "signall100-"..radio, "inform", "Радио "..radio.." е в сигнал 100!")
        end
    end
    if not type then
...
RegisterNUICallback("dispatchNotif", function(data, cb)
    ...
    if mentioned then
        ...
        TriggerEvent('erp_phone:sendNotification', {img = info['profilepic'], title = "Диспач (Mention)", content = info['message'], time = 7500, customPic = true })

        PlaySoundFrontend(-1, "SELECT", "HUD_FRONTEND_DEFAULT_SOUNDSET", false)
        PlaySoundFrontend(-1, "Event_Start_Text", "GTAO_FM_Events_Soundset", 0)
    else
        TriggerEvent('erp_phone:sendNotification', {img = info['profilepic'], title = "Диспач ("..info['name']..")", content = info['message'], time = 5000, customPic = true })
    end
    cb(true)
end)
