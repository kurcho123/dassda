...
function timeAgo(dateParam) {
  ...
  if (seconds < 5) {
    ...
    return `Преди няколко секудни`;
  } else if (seconds < 90) {
    return "Преди минута";
  } else if (minutes < 60) {
    return `Преди няколко минути`;
  } else if (isToday) {
    return getFormattedDate(date, "Днес");
  } else if (isYesterday) {
...
$(document).ready(() => {
  ...
  $(".profile-items").on("click", ".profile-item", async function () {
    ...
    $(".manage-profile-editing-title").html(`В момента редактирате ${result["firstname"]} ${result["lastname"]}`);
    ...
    let licencesHTML = '<div style="color: #fff; text-align:center;">Няма лицензи</div>';
    let tagsHTML = '<div style="color: #fff; text-align:center;">Няма тагове</div>';
    let convHTML = '<div style="color: #fff; text-align:center;">Чисто досие</div>';
    let vehHTML = '<div style="color: #fff; text-align:center;">Няма автомобили</div>';
    let galleryHTML = '<div style="color: #fff; text-align:center;">Няма снимки</div>';
    let propertyHTML = '<div style="color: #fff; text-align:center;">Няма имоти</div>';
    ...
    if (canCreateBulletin == 0) {
      ...
      $(".bulletin-items-continer")
        .prepend(`<div class="bulletin-item" data-id="${canCreateBulletin}">
                ...
                <div class="bulletin-date">${MyName} - Току-що</div>
                </div>
            </div>`);
    } else {
  ...
  $(".manage-profile-save").click(function () {
    if (canSaveProfile == true) {
      ...
      setTimeout(() => {
        $(".manage-profile-save").empty();
        $(".manage-profile-save").html("Запази");
        canSaveProfile = true;
      }, 750);
      ...
      setTimeout(() => {
        ...
        $(".tags-holder")
          .find("div")
          .each(function () {
            if ($(this).text() != "" && $(this).text() != "Няма тагове") {
              tags.push($(this).text());
            }
        });
      ...
      if (canSaveProfile == true) {
            ...
            .each(function (index) {
              if ($(this).data("id") == cid) {
                if ($(this).hasClass("green-tag")) {
                  if ($(this).text() == "Виновен") {
                    guilty = true;
                  }
                  if ($(this).text() == "Издирване") {
                    warrant = true;
                  }
                  if ($(this).text() == "Обработен") {
                    processed = true;
                  }
                  if ($(this).text() == "Съучастник") {
                    isassociated = true;
                  }
                }
              }
        ...
        setTimeout(() => {
          if (canRefreshIncidents == true) {
            ...
            setTimeout(() => {
              $(".incidents-search-refresh").empty();
              $(".incidents-search-refresh").html("Опресни");
  ...
  $(".manage-incidents-title-holder").on(
    ...
    function () {
      let tempalte =
      "Описание:\n\n· [Редови номер на участвалите полицаи.]:\n· [Код на сигнала, дата, час, локация.]:\n· [Подробен рапорт.]:\n· [Обвинения: /Член/ове/.:\n· [Конфискувани незаконни или незаконно придобити вещи от задържания.]:\n· [Конфискувани вещи, които трябва да се върнат след престой в затвора.]:\n· [Задържаният пледира: /Виновен, Невинен, Няма възражения/]:";
      $("#manage-incidents-title-input").val(
        "Име - Обвинения - " + $(".date").html()
      );
      ...
      $(".manage-incidents-title-holder").prepend(
        `
            <div class="manage-incidents-title">Управление на инцидент</div>
      ...
      $(".manage-incidents-editing-title").html(
        "В момента създавате нов инцидент"
      );
  ...
  $(".manage-bolos-title-holder").on(
    ...
    ".manage-bolos-new",
    function () {
      //if ($(".manage-bolos-editing-title").html() == 'В момента създавате нов рапорт') {
      //$(".manage-bolos-new").effect("shake", { times: 2, distance: 2 }, 500)
      //} else {
      var template = "";
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        template =
        " • Епикриза ID: [Номер на епикриза.]\n\n • Предстой в болницата: [Времетраене.]\n\n • Операция: [Да/Не]\n\n • Наранявание/Заболявания: [Въведете наранявания.]\n\n • Лечение: [Въведете списък с медикаменти.]\n\n • Обща сума: [Въведете сума.]";
      }
      $(".manage-bolos-editing-title").html(
        "В момента създавате нов рапорт"
      );
  ...
  $(".bolos-search-refresh").click(function () {
    if (canRefreshBolo == true) {
      ...
      setTimeout(() => {
        $(".bolos-search-refresh").empty();
        $(".bolos-search-refresh").html("Опресни");
        canRefreshBolo = true;
    ...
    function () {
      ...
      $(".incidents-ghost-holder").prepend(
        `
          ...
          this
        ).data("cid")}">Издирване</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Виновен</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Обработен</div>
                    <div class="associated-incidents-user-tag red-tag" data-id="${$(
          this
        ).data("cid")}">Съучастник</div>
                </div>
          ...
          "cid"
        )}"><img src="https://i.imgur.com/h7S5f9J.png"> <input placeholder="Нанесете глобата тук..." value="0" class="fine-amount" data-id="${$(
          this
        ).data("cid")}" type="number"></div>
                <div class="manage-incidents-title-tag" data-id="${$(this).data(
          "cid"
          ...
          "cid"
        )}"><img src="https://i.imgur.com/9Xn6xXK.png"> <input placeholder="Нанесете месеците тук..." value="0" class="sentence-amount" data-id="${$(
          this
        ).data("cid")}" type="number"></div>
            </div>
            `
  ...
  $(".reports-search-refresh").click(function () {
    if (canRefreshReports == true) {
      ...
      setTimeout(() => {
        $(".reports-search-refresh").empty();
        $(".reports-search-refresh").html("Опресни");
  ...
  $(".dispatch-comms-refresh").click(function () {
    ...
    setTimeout(() => {
      $(".dispatch-comms-refresh").empty();
      $(".dispatch-comms-refresh").html("Опресни");
      canRefreshReports = true;
  ...
  $(".manage-reports-title-holder").on(
    ...
    ".manage-reports-new",
    function () {
      //if ($(".manage-bolos-editing-title").html() == 'В момента редактирате рапорт') {
      //$(".manage-bolos-new").effect("shake", { times: 2, distance: 2 }, 500)
      //} else {
      let template = "";
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
            ...
            `
                            <div class="profile-item" data-id="0">
                                ...
                                <div style="display: flex; flex-direction: column;">
                                    <div class="profile-item-title">Няма резултат от търсенето</div>
                                    </div>
                            ...
                            <div style="display: flex; flex-direction: column;">
                                <div class="dmv-item-title">${value.model}</div>
                                    <div class="dmv-tags">
                                        ...
                                        <div class="dmv-tag ${impound}">Конфискуване</div>
                                        <div class="dmv-tag ${bolo}">Издирване</div>
                                        <div class="dmv-tag ${stolen}">Откраднат</div>
                                        <div class="dmv-tag ${codefive}">Код 5</div>
                                    </div>
                                </div>
                                <div class="dmv-bottom-info">
                                    <div class="dmv-id">Рег. номер: ${value.plate} · Собственик: ${value.owner}</div>
                                </div>
                            </div>
                        </div>
                    `;
        ...
        args = [
          {
            ...
            icon: "fas fa-check",
            text: "Маркирайте като открадната",
            info: plate,
            status: "",
          },
        ];
  ...
  function JobColors(sentJob) {
    if (sentJob) {
      if (PoliceJobs[sentJob] !== undefined) {
        ...
        $(".header-title").html("ПОЛИЦИЯ НА LOS SANTOS");
        $(".bolo-nav-item").html("Издирвани МПС-та");
        $(".bolos-search-title").html("Издирвани МПС-та");
        $("#bolos-search-input").attr("placeholder", "Търсете издирване...");
        $(".manage-bolos-title").html("Издирване");
        $(".manage-bolos-editing-title").html(
          "В момента създавате нов рапорт"
        );
        $(".boloplate-title").html("Рег. номер");
        $(".boloowner-title").html("Собственик");
        $(".boloindividual-title").html("Пасажери");
        $("#boloplate").attr("placeholder", "Въведете рег. номер..");
        $("#bolodetail").attr(
          "placeholder",
          "Детайли по издирването..."
        );
        $("#boloowner").attr(
          "placeholder",
          "Въведете собственик..."
        ...
        $(".quote-span").html("Отдел бира и пържоли.");
        $(".incidents-nav-item").show();
        $(".bolo-nav-item").show();
        $(".dmv-nav-item").show();
        $(".dispatch-title-ofsomesort").html("Връзка с диспач");
        ...
        $(".header-title").html("МЕДИЦИНСКИ ЦЕНТЪР ПИРОГОВ");
        $(".quote-span").html(
          "По-добре късно, отколкото никога."
        );
        $(".bolo-nav-item").html("Епикризи");
        $(".bolos-search-title").html("Постъпили пациенти");
        $("#bolos-search-input").attr(
          "placeholder",
          "Търсете чекирани..."
        );
        $(".manage-bolos-title").html("Епикриза");
        $(".manage-bolos-editing-title").html(
          "Създаване на нова епикриза"
        );
        $(".boloplate-title").html("ЕГН на пациент");
        $(".boloowner-title").html("Лекар");
        $(".boloindividual-title").html("Пациент");
        $("#boloplate").attr(
          "placeholder",
          "Въведете ЕГН на пациент..."
        );
        $("#bolodetail").attr(
          "placeholder",
          "Тук ще бъде генериран шаблон..."
        );
        $("#boloowner").attr(
          "placeholder",
        ...
        $("#boloindividual").attr(
          "placeholder",
          "Въведете име на пациента..."
        );
        $(".incidents-nav-item").hide();
        $(".dmv-nav-item").hide();
        $("#reports-officers-involved-tag-title").html("Участвали медици");
        $(".dispatch-title-ofsomesort").html("Диспач");
        $(".dispatch-comms-container").fadeIn(0);
        $(".manage-profile-name-input-1").attr("readonly", true);
        $(".manage-profile-name-input-2").attr("readonly", true);
        $(".roster-iframe").attr("src", rosterLink);
        ...
        $(".header-title").html("ОБЩИНА LOS SANTOS");
        $(".quote-span").html("Всъщност безполезно.");
        $(".dmv-nav-item").hide();
        $(".dmv-nav-item active-nav").hide();
        $(".bolo-nav-item").hide();
        $(".incidents-nav-item").hide();
        $(".dispatch-title-ofsomesort").html("Дъска за съобщения");
        ...
        $(".incidents-page-container").css("display", "none");
        $(".roster-iframe").attr("src", rosterLink);
    ...
    /* } else if (eventData.type == "bulletin") {
      ...
      $.each(eventData.data, function (index, value) {
        $('.warrants-items').prepend(`<div class="warrants-item" data-cid=${value.cid} data-id=${value.linkedincident}><div style="display: flex; flex-direction: column; margin-top: 2.5px; margin-left: 0.75vh; width: 100%;">
            ...
            <div class="warrant-bottom-info">
                <div class="warrant-id">ID на инцидент: ${value.linkedincident}</div>
            </div>
        </div></div>`)
    })
      ...
      if (value && IsValidJobForDispatch(value.job, eventData.data.playerJob)) {
        ...
        if (value["automaticGunfire"] == true) {
          DispatchItem += `<div class="call-bottom-info"><span class="fab fa-blackberry"></span>Автоматична стрелба</div>`;
        }
        ...
        $(".incidents-items").append(
          `<div class="incidents-item" data-id="${value.id}">
                    <div class="incidents-top-holder">
                        ...
                        <div class="incedent-report-name">Рапорт</div>
                    </div>
                    <div class="incidents-bottom-holder">
                        <div class="incedent-report-id">No: ${value.id}</div>
      ...
      $(".manage-incidents-editing-title").html(
        "В момента редактирате инцидент " + table["id"]
      );
        ...
        if (value.associated == 1) {
          $(".incidents-ghost-holder").prepend(
            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="associated-incidents-user-tags-holder">
                                <div class="associated-incidents-user-tag ${warrantTag}" data-id="${value.cid}">Издирване</div>
                                <div class="associated-incidents-user-tag ${guiltyTag}" data-id="${value.cid}">Виновен</div>
                                <div class="associated-incidents-user-tag ${processedTag}" data-id="${value.cid}">Обработен</div>
                                <div class="associated-incidents-user-tag ${associatedTag}" data-id="${value.cid}">Съучастник</div>
                            </div>
                            <div class="associated-incidents-user-holder" data-name="${value.cid}" style="display:none;">
                            </div>
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Препоръчителна глоба</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Препоръчителен затвор</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Глоба</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}" style="display:none;">Затвор</div>
          ...
          $(".incidents-ghost-holder").prepend(
            `<div class="associated-incidents-user-container" data-id="${value.cid}">
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Препоръчителна глоба</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Препоръчителен затвор</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Глоба</div>
                            ...
                            <div class="manage-incidents-title-tag" data-id="${value.cid}">Затвор</div>
          ...
          `
                    <div class="icidents-person-search-item" data-info="${name} (#${value.id})" data-cid="${value.id}" data-name="${name}">
                        ...
                        <div class="icidents-person-search-item-right">
                            <div class="icidents-person-search-item-right-cid-title">ЕГН</div>
                            <div class="icidents-person-search-item-right-cid-input"><span class="fas fa-id-card"></span> ${value.id}</div>
                            <div class="icidents-person-search-item-right-name-title">Име</div>
      ...
      let table = eventData.data;
      $(".manage-bolos-editing-title").html(
        "В момента редактирате рапорт " + table["id"]
      );

      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        $(".manage-bolos-editing-title").html(
          "В момента редактирате чекиране " + table["id"]
        );
      }
      ...
      let table = eventData.data;
      var reportName = "Издирване на МПС";
      canSearchForProfiles = true;
      $(".bolos-items").empty();
      if ($(".badge-logo").attr("src") == "img/ems_badge.png") {
        reportName = "Епикриза";
      ...
      $(".manage-bolos-editing-title").html(
        "В момента редактирате рапорт " + id
      );
      ...
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${impound} impound-tag">Конфискуване</div>`
      );
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${bolo}">Издирване</div>`
      );
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${codefive} code5-tag">Код 5</div>`
      );
      $(".vehicle-tags").append(
        `<div class="vehicle-tag ${stolen} stolen-tag">Откраднат</div>`
      );
      $(".vehicle-info-imageurl-input").val(table["image"]);
