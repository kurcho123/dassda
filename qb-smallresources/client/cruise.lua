...
local handBrakeCooldown = 0
CreateThread(function()
    while true do
        Wait(1)
        if IsPedInAnyVehicle(PlayerPedId(), false) then
            if IsControlJustPressed(0, 76) == 1 then
                handBrakeCooldown = 100
            end
            if handBrakeCooldown > 0 then
                handBrakeCooldown = handBrakeCooldown - 1
            end
        end
    end
end)
            ...
            CreateThread(function()
                while CruisedSpeed > 0 and IsInVehicle() == Player do
                    ...
                    if not IsTurningOrHandBraking() and
                        ...
                        GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId(), false)) == 1 and
