...
Citizen.CreateThread(function()
    for k, v in pairs (Config.Stores) do
        if Config.Stores[k].shopType == "clothing" then
            ...
            AddTextComponentString("Магазин за дрехи")
        ...
        if Config.Stores[k].shopType == "barber" then
            ...
            BeginTextCommandSetBlipName("STRING")
            AddTextComponentString("Салон за красота")
		...
		local dist = #(pos - Config.Stores[k].coords)
                if dist < 30 then
                    if not creatingCharacter then
                        ...
                        if dist < 5 then
                            if Config.Stores[k].shopType == "clothing" then
                                 DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Шопинг')
                            elseif Config.Stores[k].shopType == "barber" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Бръснарница')
                            elseif Config.Stores[k].shopType == "surgeon" then
                                DrawText3Ds(Config.Stores[k].coords.x, Config.Stores[k].coords.y, Config.Stores[k].coords.z + 1.25, '~g~E~w~ - Пластична хирургия')
                            end
                            if IsControlJustPressed(0, 38) then -- E
                                if Config.Stores[k].shopType == "clothing" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "character", label = "Дрехи", selected = true},
                                        {menu = "accessoires", label = "Аксесоари", selected = false}
                                    })
                                elseif Config.Stores[k].shopType == "barber" then
                                    customCamLocation = nil
                                    openMenu({
                                        {menu = "clothing", label = "Коса", selected = true},
                                    })
                                elseif Config.Stores[k].shopType == "surgeon" then
                                    customCamLocation = nil
                                    openMenu({
                    ...
                    if not creatingCharacter then
                        ...
                        if dist < 2 then
                            if PlayerData.job.name == Config.ClothingRooms[k].requiredJob then
                                DrawText3Ds(Config.ClothingRooms[k].coords.x, Config.ClothingRooms[k].coords.y, Config.ClothingRooms[k].coords.z + 0.3, '~g~E~w~ - Вижте облекло')
                                if IsControlJustPressed(0, 38) then -- E
                                    ...
                                    QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                        openMenu({
                                            {menu = "roomOutfits", label = "Облекла", selected = true, outfits = Config.Outfits[PlayerData.job.name][gender]},
                                            {menu = "myOutfits", label = "Запазени облекла", selected = false, outfits = result},
                                            {menu = "character", label = "Дрехи", selected = false},
                                            {menu = "accessoires", label = "Аксесоари", selected = false}
                                        })
                                    end)
                                end
                                ...
                                if PlayerData.gang.name == Config.ClothingRooms[k].requiredJob then
                                    ...
                                    if IsControlJustPressed(0, 38) then -- E
                                        ...
                                        QBCore.Functions.TriggerCallback('qb-clothing:server:getOutfits', function(result)
                                            openMenu({
                                                {menu = "roomOutfits", label = "Облекла", selected = true, outfits = Config.Outfits[PlayerData.gang.name][gender]},
                                                {menu = "myOutfits", label = "Запазени облекла", selected = false, outfits = result},
                                                {menu = "character", label = "Дрехи", selected = false},
                                                {menu = "accessoires", label = "Аксесоари", selected = false}
                                            })
...
AddEventHandler('qb-clothing:client:openMenu', function()
    ...
    openMenu({
        {menu = "character", label = "Дрехи", selected = true},
        {menu = "clothing", label = "Визия", selected = false},
        {menu = "accessoires", label = "Аксесоари", selected = false}
    })
end)
...
RegisterNUICallback('TrackerError', function()
    QBCore.Functions.Notify("Не можете да свалите гривната си от глезена ..", "error")
end)
