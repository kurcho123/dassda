    ...
    window.addEventListener('message', function (event) {
        ...
        if (data.action == "ui") {
            ...
            if (data.toggle) {
                ...
                var originalText = "Извличане на данни за персонажа";
                ...
                var DotsInterval = setInterval(function() {
                    ...
                    if (loadingProgress == 3) {
                        originalText = "Проверка на данните за персонажа"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 4) {
                        originalText = "Извличане на знаци"
                        $("#loading-text").html(originalText);
                    }
                    if (loadingProgress == 6) {
                        originalText = "Удостоверяващи знаци"
                        $("#loading-text").html(originalText);
                    }
                    if(loadingDots == 4) {
                        $("#loading-text").html(originalText);
                    ...
                    setTimeout(function(){
                        ...
                        $(".title-screen").fadeIn(0, function() {
                            setTimeout(function() {
                                ...
                                $('.jugadores-on').html(' ' + data.players + ' МАНЯКА');
                            }, 1000);
...
function setupCharInfo(cData) {
    if (cData == 'empty') {
        $('.character-info-valid').html('<span id="no-char">Този слот още не се използва!<br><br>Все още не знаем нищо за теб!</span>');
    } else {
        var gender = "Man"
        if (cData.charinfo.gender == 1) { gender = "Woman" }
        $('.character-info-valid').html(
        '<div class="character-info-box"><span id="info-label">Име: </span><span class="char-info-js">'+cData.charinfo.firstname+' '+cData.charinfo.lastname+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Рожден ден: </span><span class="char-info-js">'+cData.charinfo.birthdate+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Пол: </span><span class="char-info-js">'+gender+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Националност: </span><span class="char-info-js">'+cData.charinfo.nationality+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Бачка в: </span><span class="char-info-js">'+cData.job.label+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Кеш: </span><span class="char-info-js">&#36; '+cData.money.cash+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Банка: </span><span class="char-info-js">&#36; '+cData.money.bank+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Номер: </span><span class="char-info-js">'+cData.charinfo.phone+'</span></div>' +
        '<div class="character-info-box"><span id="info-label">Банкова сметка: </span><span class="char-info-js">'+cData.charinfo.account+'</span></div>');
    }
}
...
function setCharactersList() {
    var htmlResult = '<div class="character-list-header"><p>Персонажи</p></div>'
    for (let i = 1; i <= NChar; i++) {
        htmlResult += '<div class="character" id="char-'+ i +'" data-cid=""><span id="slot-name">Свободен слот<span id="cid"></span></span></div>'
    }
    htmlResult += '<div class="character-btn" id="play"><p id="play-text">Избери персонаж</p></div><div class="character-btn" id="delete"><p id="delete-text">Избери персонаж</p></div>'
