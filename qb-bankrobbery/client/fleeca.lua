...
function openLocker(bankId, lockerId) -- Globally Used
    ...
    if bankId == "paleto" then
        ...
        if hasItem then
            ...
            IsDrilling = true
            QBCore.Functions.Progressbar("open_locker_drill", Lang:t("general.breaking_open_safe"), math.random(38000, 160000), false, true, {
    ...
    elseif bankId == "pacific" then
        ...
        QBCore.Functions.Progressbar("open_locker", Lang:t("general.breaking_open_safe"), math.random(38000, 160000), false, true, {
            disableMovement = true,
            disableCarMovement = true,
            disableMouse = false,
            disableCombat = true,
            ...
            if CurrentCops >= Config.MinimumFleecaPolice then
                if not Config.SmallBanks[closestBank]["isOpened"] then
                    ...
                    if hasItem then
                        ...
                        loadAnimDict("anim@gangops@facility@servers@")
                        TaskPlayAnim(ped, 'anim@gangops@facility@servers@', 'hotwire', 3.0, 3.0, -1, 1, 0, false, false, false)
                        QBCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(15000, 30000), false, true, {
                            disableMovement = true,
                            disableCarMovement = true,
                            disableMouse = false,
                            disableCombat = true,
                            ...
                            TriggerEvent("mhacking:start", math.random(6, 7), math.random(17, 25), OnHackDone)
