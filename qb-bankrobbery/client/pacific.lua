        ...
        if not isBusy then
            if CurrentCops >= Config.MinimumPacificPolice then
                if not Config.BigBanks["pacific"]["isOpened"] then
                    ...
                    QBCore.Functions.Progressbar("security_pass", Lang:t("general.validating_bankcard"), math.random(15000, 30000), false, true, {
                ...
                if not Config.BigBanks["pacific"]["isOpened"] then
                    ...
                    if hasItem then
                        ...
                        QBCore.Functions.Progressbar("hack_gate", Lang:t("general.connecting_hacking_device"), math.random(15000, 30000), false, true, {
                            ...
                            TriggerEvent("mhacking:start", math.random(5, 9), math.random(15, 25), OnHackPacificDone)
                            if copsCalled or not Config.BigBanks["pacific"]["alarm"] then return end
                            TriggerServerEvent("qb-bankrobbery:server:callCops", "pacific", 0, pos)
                            copsCalled = true
                        end, function() -- Cancel
