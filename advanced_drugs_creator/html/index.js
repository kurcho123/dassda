...
function isThereAnyErrorInForm(event) {
	if (!event.target.checkValidity()) {
		...
		swal("ERROR", "There is a red field somewhere!", "error");
		return true;
	} else {
...
function startTimedProgressBar(time, text) {
	let progressBarDiv = $("#drugs-creator-progressbar-div");
	let progressBar = $("#drugs-creator-progressbar")

    $("#drugs-creator-progressbar-text").text(text);
...
function stopProgressBar() {
	if(progressBarInterval) {
		clearInterval(progressBarInterval);
		$("#drugs-creator-progressbar-div").fadeOut();
	}
}
/* Progress bar END */
