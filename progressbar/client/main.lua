...
function Process(action, start, tick, finish)
    TriggerEvent('antidupe:disableinv', true)
    local other = 'otherplayer-' .. GetPlayerServerId(PlayerId())
    TriggerServerEvent('antidupe:server:checkifrob', other)
	ActionStart()
    Action = action
    ...
    if not IsEntityDead(ped) or Action.useWhileDead then
        if not isDoingAction then
            ...
            CreateThread(function ()
                ...
                if finish ~= nil then
                    TriggerEvent('antidupe:disableinv', false)
                    finish(wasCancelled)
                end
            end)
        else
