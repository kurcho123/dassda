...
-----------------Anti Duplication Glitch-------------------
local blockInv = false

RegisterNetEvent('antidupe:disableinv', function(state)
    blockInv = state
end)

RegisterNetEvent('antidupe:client:checkifrob', function(other)
    if currentOtherInventory == other then
        blockInv = true
        Wait(3000)
        blockInv = false
    end
...
CreateThread(function()
    while true do
        Wait(5)
        if blockInv == true and inInventory == true then
            closeInventory()
            local msg = GetPlayerName(GetPlayerIndex()) .. ' is probably trying to duplicate an item!'
            -- TriggerServerEvent('antidupe:logconsole', msg)
            TriggerServerEvent('qb-log:server:CreateLog', 'invdupe', 'Inventory Dupe', 'deafult', msg, true)
        end
    end
end)
                ...
                if vehicle ~= 0 and vehicle ~= nil then
                    ...
                    if #(pos - trunkpos) < 2.0 and not IsPedInAnyVehicle(ped) then
                        if GetVehicleDoorLockStatus(vehicle) < 2 then
                            ...
                            QBCore.Functions.Notify("Колата е заключена", "error")
                            return
                        end
                    else
                        CurrentVehicle = nil
...
RegisterNUICallback('combineWithAnim', function(data)
    ...
    QBCore.Functions.Progressbar("combine_anim", animText, animTimeout, false, true, {
        ...
        StopAnimTask(ped, aDict, aLib, 1.0)
        QBCore.Functions.Notify("Стана проблем!", "error")
    end)
end)
...
RegisterNUICallback("GiveItem", function(data)
    ...
    if player ~= -1 and distance < 3 then
        if (data.inventory == 'player') then
            ...
            QBCore.Functions.Notify("Вие не притежавате този итем!", "error")
        end
    else
        QBCore.Functions.Notify("Няма никой до теб!", "error")
    end
end)
