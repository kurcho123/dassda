...
local toggle_rappel = 52 -- control id to rappel out of the heli. Default: INPUT_DUCK (X)
...
-- Script starts here
local helicam = false
local polmav_hash = GetHashKey("fibfrogger")
			...
			if IsHeliHighEnough(heli) then
				...
				if IsControlJustPressed(0, toggle_rappel) then -- Initiate rappel
					...
					if GetPedInVehicleSeat(heli, 1) == lPed or GetPedInVehicleSeat(heli, 2) == lPed then
						...
						AddTextComponentString("~r~Не можеш да се спуснеш от това място")
