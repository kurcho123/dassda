            ...
            for safe,_ in pairs(Config.Safes) do
                ...
                if dist < 3 then
					...
					local s1, s2 = GetStreetNameAtCoord(pos.x, pos.y, pos.z)
                                        ...
                                        TriggerServerEvent('police:server:policeAlert', 'Обир на магазин')
                                        copsCalled = true
                                    end
                                else
...
local function PoliceCall()
    local chance = 95
    if GetClockHours() >= 0 and GetClockHours() <= 0 then
        chance = 95
    end
...
RegisterNetEvent('qb-storerobbery:client:robberyCall', function(type, key, streetLabel, coords)
    if PlayerJob.name == "police" and onDuty then
        ...
        TriggerEvent('qb-policealerts:client:AddPoliceAlert', {
            timeOut = 15000,
            alertTitle = "Стартиран е ОБИР НА МAГАЗИН",
            coords = {
                x = coords.x,
                y = coords.y,
                z = coords.z,
        ...
        AddTextComponentString("Стартиран е ОБИР НА МAГАЗИН")
